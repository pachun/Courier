module Courier
  class Base < CoreData::Model
    ...
    def self.where(scope)
      ...
      puts "self.to_s is \"#{self.to_s}\""
      fetch.setPredicate(scope)
      error = Pointer.new(:object)

      # if scope.class == NSCompoundPredicate
      #   puts "\n#{scope} subpredicates are:"
      #   scope.subpredicates.each{ |sp| puts sp.inspect }
      # end
    ...
    def self.scope(name, scope)
      ...
      class_constant = self
      puts "class constant = #{class_constant}"
